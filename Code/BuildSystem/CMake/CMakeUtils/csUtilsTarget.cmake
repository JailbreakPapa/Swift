# #####################################
# ## CS_create_target(<LIBRARY | APPLICATION> <target-name> [NO_PCH] [NO_UNITY] [NO_QT] [ALL_SYMBOLS_VISIBLE] [EXCLUDE_FOLDER_FOR_UNITY <relative-folder>...])
# #####################################

macro(CS_create_target TYPE TARGET_NAME)
	CS_apply_build_filter(${TARGET_NAME})

	set(ARG_OPTIONS NO_PCH NO_UNITY NO_QT NO_CS_PREFIX ENABLE_RTTI NO_WARNINGS_AS_ERRORS NO_CONTROLFLOWGUARD ALL_SYMBOLS_VISIBLE NO_DEBUG)
	...
	if(ARG_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "CS_create_target: Invalid arguments '${ARG_UNPARSED_ARGUMENTS}'")
	endif()
	...
	if(DEFINED ARG_MANUAL_SOURCE_FILES)
		...
		CS_glob_source_files(${CMAKE_CURRENT_SOURCE_DIR} ALL_SOURCE_FILES)
	endif()

	# SHARED_LIBRARY means always shared
	# LIBRARY means SHARED_LIBRARY when CS_COMPILE_ENGINE_AS_DLL is on, otherwise STATIC_LIBRARY
	if((${TYPE} STREQUAL "LIBRARY") OR(${TYPE} STREQUAL "STATIC_LIBRARY") OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		if((${CS_COMPILE_ENGINE_AS_DLL} AND(${TYPE} STREQUAL "LIBRARY")) OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		...
		if(NOT ARG_NO_CS_PREFIX)
			CS_add_output_CS_prefix(${TARGET_NAME})
		endif()

		CS_set_library_properties(${TARGET_NAME})
		CS_uwp_fix_library_properties(${TARGET_NAME} "${ALL_SOURCE_FILES}")
	...
	elseif(${TYPE} STREQUAL "APPLICATION")
		...
		if(CS_CMAKE_PLATFORM_ANDROID)
			# All CS applications must include the native app glue implementation
		...
		CS_uwp_add_default_content(${TARGET_NAME})

		CS_set_application_properties(${TARGET_NAME})

	else()
		message(FATAL_ERROR "CS_create_target: Missing argument to specify target type. Pass in 'APP' or 'LIB'.")
	endif()
	...
	if(NOT ${ARG_NO_PCH})
		CS_auto_pch(${TARGET_NAME} "${ALL_SOURCE_FILES}" ${ARG_EXCLUDE_FROM_PCH_REGEX})
	endif()
	...
	if((NOT ANDROID) OR(NOT CS_CMAKE_INSIDE_VS))
		CS_set_default_target_output_dirs(${TARGET_NAME})
	endif()
	...
	if(${TYPE} STREQUAL "APPLICATION")
		CS_android_add_default_content(${TARGET_NAME})
	endif()

	CS_add_target_folder_as_include_dir(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR})

	CS_set_common_target_definitions(${TARGET_NAME})

	CS_set_build_flags(${TARGET_NAME} ${ARGN})

	# On linux we want all symbols to be hidden by default. We manually "export" them.
	if(CS_COMPILE_ENGINE_AS_DLL AND CS_CMAKE_PLATFORM_LINUX AND NOT ARG_ALL_SYMBOLS_VISIBLE)
		target_compile_options(${TARGET_NAME} PRIVATE "-fvisibility=hidden")
	endif()

	CS_set_project_ide_folder(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR})

	# Pass the windows sdk include paths to the resource compiler when not generating a visual studio solution.
	if(CS_CMAKE_PLATFORM_WINDOWS AND NOT CS_CMAKE_GENERATOR_MSVC)
		set(RC_FILES ${ALL_SOURCE_FILES})
		list(FILTER RC_FILES INCLUDE REGEX ".*\\.rc$")

		if(RC_FILES)
			set_source_files_properties(${RC_FILES}
				PROPERTIES COMPILE_FLAGS "/I\"C:/Program Files (x86)/Windows Kits/10/Include/${CS_CMAKE_WINDOWS_SDK_VERSION}/shared\" /I\"C:/Program Files (x86)/Windows Kits/10/Include/${CS_CMAKE_WINDOWS_SDK_VERSION}/um\""
			)
		endif()
	endif()
	...
	if(NOT ${ARG_NO_QT})
		CS_qt_wrap_target_files(${TARGET_NAME} "${ALL_SOURCE_FILES}")
	endif()

	CS_ci_add_to_targets_list(${TARGET_NAME} C++)

	if(NOT ${ARG_NO_UNITY})
		CS_generate_folder_unity_files_for_target(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR} "${ARG_EXCLUDE_FOLDER_FOR_UNITY}")
	endif()
