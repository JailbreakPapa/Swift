...
# #####################################
# ## CS_retrieve_target_pch(<target> <pch-name>)
# #####################################
function(CS_retrieve_target_pch TARGET_NAME PCH_NAME)
...
# #####################################
# ## CS_pch_use(<pch-header> <cpp-files>)
# #####################################
function(CS_pch_use PCH_H TARGET_CPPS)
	if(NOT CS_USE_PCH OR CS_CMAKE_GENERATOR_MSVC)
...
# #####################################
# ## CS_pch_create(<pch-header> <cpp-file>)
# #####################################
function(CS_pch_create PCH_H TARGET_CPP)
	if(NOT CS_CMAKE_GENERATOR_MSVC)
		return()
	endif()
...
# #####################################
# ## CS_find_pch_in_file_list(<files> <out-pch-name>)
# #####################################
function(CS_find_pch_in_file_list FILE_LIST PCH_PATH PCH_NAME)
...
# #####################################
# ## CS_auto_pch(<target> <files> [<file-exclude-regex> ... ])
# #####################################
function(CS_auto_pch TARGET_NAME FILES)
	foreach(EXCLUDE_PATTERN ${ARGN})
		list(FILTER FILES EXCLUDE REGEX ${EXCLUDE_PATTERN})
	endforeach()

	CS_find_pch_in_file_list("${FILES}" PCH_PATH PCH_NAME)
	
	if(NOT PCH_NAME)
		return()
	endif()

	CS_pch_create("${PCH_NAME}.h" ${PCH_PATH})

	CS_pch_use("${PCH_NAME}.h" "${FILES}")

	CS_set_target_pch(${TARGET_NAME} ${PCH_NAME})
endfunction()
