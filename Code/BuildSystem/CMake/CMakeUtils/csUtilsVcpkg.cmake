...
function(CS_vcpkg_init)
	...
	if(CMAKE_TOOLCHAIN_FILE)
		...
		get_filename_component(CS_VCPKG_ROOT "${CMAKE_TOOLCHAIN_FILE}" DIRECTORY)
		get_filename_component(CS_VCPKG_ROOT "${CS_VCPKG_ROOT}" DIRECTORY)
		get_filename_component(CS_VCPKG_ROOT "${CS_VCPKG_ROOT}" DIRECTORY)

	else()
		if(DEFINED ENV{VCPKG_ROOT})
			set(CS_VCPKG_ROOT "$ENV{VCPKG_ROOT}")
			message(STATUS "EnvVar VCPKG_ROOT is specified, using that.")
		else()
			set(CS_VCPKG_ROOT "${CMAKE_CURRENT_SOURCE_DIR}/vcpkg")
			message(STATUS "EnvVar VCPKG_ROOT is not specified, using '${CMAKE_CURRENT_SOURCE_DIR}/vcpkg'.")
		endif()

		if(NOT EXISTS "${CS_VCPKG_ROOT}/vcpkg.exe" OR NOT EXISTS "${CS_VCPKG_ROOT}/scripts/buildsystems/vcpkg.cmake")
		...
		set(CMAKE_TOOLCHAIN_FILE ${CS_VCPKG_ROOT}/scripts/buildsystems/vcpkg.cmake CACHE STRING "" FORCE)
		message(STATUS "Forcing CMAKE_TOOLCHAIN_FILE to point to '${CMAKE_TOOLCHAIN_FILE}'")
	endif()

	message(STATUS "CS_VCPKG_ROOT is '${CS_VCPKG_ROOT}'")
	set_property(GLOBAL PROPERTY "CS_VCPKG_ROOT" ${CS_VCPKG_ROOT})
endfunction()

# #####################################
# ## CS_vcpkg_install(<packages>)
# #####################################
function(CS_vcpkg_install PACKAGES)
	CS_cmake_init()

	get_property(CS_VCPKG_ROOT GLOBAL PROPERTY "CS_VCPKG_ROOT")
	...
	foreach(PACKAGE ${PACKAGES})
		...
		execute_process(COMMAND "${CS_VCPKG_ROOT}/vcpkg.exe" install "${PACKAGE}:${VCPKG_TARGET_TRIPLET}" WORKING_DIRECTORY "${CS_VCPKG_ROOT}")
	endforeach()
endfunction()
