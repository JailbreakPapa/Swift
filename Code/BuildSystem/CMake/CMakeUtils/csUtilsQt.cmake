...
set(CS_ENABLE_QT_SUPPORT ON CACHE BOOL "Whether to add Qt support.")
set(CS_QT_DIR $ENV{QTDIR} CACHE PATH "Directory of the Qt installation")

# #####################################
# ## CS_requires_qt()
# #####################################
macro(CS_requires_qt)
	CS_requires(CS_ENABLE_QT_SUPPORT)
endmacro()
...
macro(CS_find_qt)
	...
	if(CS_ENABLE_QT_SUPPORT)
		if(CS_QT_DIR)
			find_package(Qt6 COMPONENTS ${CS_QT_COMPONENTS} REQUIRED PATHS ${CS_QT_DIR})
		else()
			find_package(Qt6 COMPONENTS ${CS_QT_COMPONENTS} REQUIRED)
		endif()
	endif()
	...
	if(CS_CMAKE_PLATFORM_WINDOWS AND CS_CMAKE_COMPILER_CLANG)
...
# ## CS_prepare_find_qt()
# #####################################
function(CS_prepare_find_qt)
	set(CS_CACHED_QT_DIR "CS_CACHED_QT_DIR-NOTFOUND" CACHE STRING "")
	mark_as_advanced(CS_CACHED_QT_DIR FORCE)
	...
	if(CS_CMAKE_PLATFORM_WINDOWS_DESKTOP AND CS_CMAKE_ARCHITECTURE_64BIT)
		# Upgrade from Qt5 to Qt6 if the CS_QT_DIR points to a previously automatically downloaded Qt5 package.
		if("${CS_QT_DIR}" MATCHES ".*Qt-5\\.13\\.0-vs141-x64")
			set(CS_QT_DIR "CS_QT_DIR-NOTFOUND" CACHE PATH "Directory of the Qt installation" FORCE)
		endif()
	
		if(CS_CMAKE_ARCHITECTURE_64BIT)
			set(CS_SDK_VERSION "${CS_CONFIG_QT_WINX64_VERSION}")
			set(CS_SDK_URL "${CS_CONFIG_QT_WINX64_URL}")
		endif()

		if((CS_QT_DIR STREQUAL "CS_QT_DIR-NOTFOUND") OR(CS_QT_DIR STREQUAL ""))
			CS_download_and_extract("${CS_SDK_URL}" "${CMAKE_BINARY_DIR}" "${CS_SDK_VERSION}")

			set(CS_QT_DIR "${CMAKE_BINARY_DIR}/${CS_SDK_VERSION}" CACHE PATH "Directory of the Qt installation" FORCE)
		endif()
	endif()

	# # Download Qt package
	# #####################
	if(NOT "${CS_QT_DIR}" STREQUAL "${CS_CACHED_QT_DIR}")
		# Need to reset qt vars now so that 'find_package' is re-executed
		set(CS_CACHED_QT_DIR ${CS_QT_DIR} CACHE STRING "" FORCE)
	...
	if(CS_QT_DIR)
		set(CMAKE_PREFIX_PATH ${CS_QT_DIR} PARENT_SCOPE)
	endif()
endfunction()

# #####################################
# ## CS_link_target_qt(TARGET <target> COMPONENTS <qt components> [COPY_DLLS])
# #####################################
function(CS_link_target_qt)
	CS_pull_all_vars()
	...
	if(FN_ARG_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "CS_link_target_qt: Invalid arguments '${FN_ARG_UNPARSED_ARGUMENTS}'")
	endif()
	...
	target_compile_definitions(${FN_ARG_TARGET} PUBLIC CS_USE_QT)
...
# #####################################
# ## CS_qt_wrap_target_moc_files(<target> <files>)
# #####################################
function(CS_qt_wrap_target_moc_files TARGET_NAME FILES_TO_WRAP)
	CS_requires_qt()
...
# #####################################
# ## CS_qt_wrap_target_qrc_files(<target> <files>)
# #####################################
function(CS_qt_wrap_target_qrc_files TARGET_NAME FILES_TO_WRAP)
	CS_requires_qt()
...
# #####################################
# ## CS_qt_wrap_target_files(<target> <files>)
# #####################################
function(CS_qt_wrap_target_files TARGET_NAME FILES_TO_WRAP)
	CS_requires_qt()

	CS_qt_wrap_target_qrc_files(${TARGET_NAME} "${FILES_TO_WRAP}")

	CS_qt_wrap_target_ui_files(${TARGET_NAME} "${FILES_TO_WRAP}")

	# in this automated method, we only MOC files that end with ".moc.h"
	list(FILTER FILES_TO_WRAP INCLUDE REGEX ".*\.moc\.h")
	CS_qt_wrap_target_moc_files(${TARGET_NAME} "${FILES_TO_WRAP}")
endfunction()
