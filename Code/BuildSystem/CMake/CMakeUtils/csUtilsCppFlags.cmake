...
function(CS_check_build_type)
	# set the default build type
	if(NOT CMAKE_BUILD_TYPE)
		set(CMAKE_BUILD_TYPE ${CS_BUILDTYPENAME_DEV} CACHE STRING "Choose the type of build, options are: None ${CS_BUILDTYPENAME_DEBUG} ${CS_BUILDTYPENAME_DEV} ${CS_BUILDTYPENAME_RELEASE}." FORCE)
	endif()
endfunction()

# #####################################
# ## CS_set_build_flags_msvc(<target>)
# #####################################
function(CS_set_build_flags_msvc TARGET_NAME)
	...
	# target_compile_options(${TARGET_NAME} PRIVATE "/W4") # too much work to fix all warnings in CS
	...
	if((CMAKE_SIZEOF_VOID_P EQUAL 4) AND CS_CMAKE_ARCHITECTURE_X86)
	...
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_RELEASE_UPPER}>:/Zo>")
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_DEV_UPPER}>:/Zo>")

	# /Ob1: Only consider functions for inlining that are marked with inline or forceinline
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_DEBUG_UPPER}>:/Ob1>")

	# /Ox: favor speed for optimizations
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_RELEASE_UPPER}>:/Ox>")

	# /Ob2: Consider all functions for inlining
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_RELEASE_UPPER}>:/Ob2>")

	# /Oi: Replace some functions with intrinsiCS or other special forms of the function
	target_compile_options(${TARGET_NAME} PRIVATE "$<$<CONFIG:${CS_BUILDTYPENAME_RELEASE_UPPER}>:/Oi>")
	...
	if(CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND CS_CMAKE_ARCHITECTURE_X86)
	...
	if(${ARG_NO_DEBUG})
		...
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${CS_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_RELEASE})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${CS_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_RELEASE})
	else()
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${CS_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_DEBUG})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${CS_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_${CS_DEV_BUILD_LINKERFLAGS}})
	endif()

	set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${CS_BUILDTYPENAME_RELEASE_UPPER} ${LINKER_FLAGS_RELEASE})

	if(CS_ENABLE_COMPILER_STATIC_ANALYSIS)
...
# #####################################
# ## CS_set_build_flags_clang(<target>)
# #####################################
function(CS_set_build_flags_clang TARGET_NAME)
	...
	if(CS_ENABLE_QT_SUPPORT)
		# Ignore any warnings caused by Qt headers
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CS_QT_DIR}\"")
	endif()

	# Ignore any warnings caused by headers inside the ThirdParty directory.
	if(CS_SUBMODULE_PREFIX_PATH)
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CMAKE_SOURCE_DIR}/${CS_SUBMODULE_PREFIX_PATH}/Code/ThirdParty\"")
	else()
...
# #####################################
# ## CS_set_build_flags_gcc(<target>)
# #####################################
function(CS_set_build_flags_gcc TARGET_NAME)
	...
	if(NOT(CMAKE_CURRENT_SOURCE_DIR MATCHES "Code/ThirdParty"))
		# Warning / Error settings for CS code
		# attributes = error if a attribute is placed incorrectly (e.g. CS_FOUNDATION_DLL)
		# unused-result = error if [[nodiscard]] return value is not handeled (CSResult)
...
# #####################################
# ## CS_set_build_flags(<target>)
# #####################################
function(CS_set_build_flags TARGET_NAME)
	...
	if(CS_CMAKE_COMPILER_MSVC)
		CS_set_build_flags_msvc(${TARGET_NAME} ${ARGN})
	endif()

	if(CS_CMAKE_COMPILER_CLANG)
		CS_set_build_flags_clang(${TARGET_NAME} ${ARGN})
	endif()

	if(CS_CMAKE_COMPILER_GCC)
		CS_set_build_flags_gcc(${TARGET_NAME} ${ARGN})
	endif()
endfunction()
